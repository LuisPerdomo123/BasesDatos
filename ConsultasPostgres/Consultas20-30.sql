...
select nom_e,def,nom_a from listado where nom_a ilike '%base%dato' order by def desc limit 1; -- Primera solución

with calificaciones_bases_datos as (select nom_e,def,nom_a from listado where lower (nom_a) ilike '%base%dato') -- Segunda solucion
select * from calificaciones_bases_datos where def = (select max (def) from calificaciones_bases_datos);


--23. Cuáles asignaturas tienen la nota definitiva más baja

select nom_a,def from listado where def = (select min (def) from listado);

--24. Quienes tienen la nota más baja de todas, en qué asignatura y con cuál profesor

select nom_e, def, nom_a, nom_p from listado order by def limit 1; -- Primera solucion

select nom_e, def, nom_a, nom_p from listado where def = (select min (def) from listado);	--Segunda solucion
...
create temporary view l_libros as  -- Primera solucion
...
with libro_referenciado_asignaturas as (select titulo, count (nom_a) cuenta from libros natural join referencia natural join asignaturas group by titulo)-- Segunda respuesta
select titulo, cuenta from libro_referenciado_asignaturas where cuenta = (select max(cuenta) from libro_referenciado_asignaturas);
...
create temporary view l_estudiante as
	select nom_e, count (distinct isbn) cuenta
...
select nom_p, count(nom_e) asig_perd from listado where def <= 3 group by nom_p;
...
select nom_p, asig_perd/(select sum(asig_perd) from l_profesores_2 )* 100 as porcentaje from l_profesores_2 ;
select * from l_profesores_3 where porcentaje = (select max(porcentaje) from l_profesores_3);
...
select * from l_asignaturas where estu = (select max(estu) from l_asignaturas);
