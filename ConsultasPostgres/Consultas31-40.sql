...
select count (*) as conteo, isbn from presta where isbn in(
	select distinct isbn list from presta
	except
	select distinct isbn from referencia natural join asignaturas) group by isbn;
select * from l_libros_1 where conteo = (select max(conteo) from l_libros_1);
...
with notas_est_promedio as (select nom_e,avg(def) promedio from listado group by nom_e) -- Tercera solucion
select * from notas_est_promedio where promedio = (select max (promedio) from notas_est_promedio);
...
select * from listado_libros_3 where cantidad = (select max (cantidad) from listado_libros_3); -- Tercera forma
...
select distinct cod_a,nom_a from referencia natural join asignaturas except 
select distinct cod_a, nom_a from presta natural join estudiantes natural join inscribe natural join asignaturas;

with lib_no_pres as (select distinct isbn from referencia except 
select distinct isbn from presta)
select distinct cod_a, nom_a from lib_no_pres natural join referencia natural join asignaturas;

with asignaturas_ref_con_libros_prestados as (select distinct cod_a from referencia where isbn in (select isbn from presta))
select distinct cod_a, nom_a from referencia natural join asignaturas 
	where cod_a not in (select * from asignaturas_ref_con_libros_prestados) order by cod_a
...
select * from prof_mas_asi where mat_dic = (select max(mat_dic) from prof_mas_asi);
